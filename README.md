...
- **考试时间顺延**：已发布考试可顺延结束时间（教师限本人创建，管理员不限），进行中试卷的个人截止时间按相同分钟数同步顺延；每次顺延留存只增不改删的调整记录（操作人、原/新结束时间、顺延分钟数、受影响试卷数）。
...
| POST | `/api/v1/exams/:id/extend` | 顺延考试结束时间（同步顺延进行中试卷的个人截止时间） | 管理员/教师 |
| GET | `/api/v1/exams/:id/extensions` | 顺延调整记录（只增不改删，按时间倒序） | 管理员/教师 |
